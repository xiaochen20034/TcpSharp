...
public class TcpSharpSocketClient<TPackageStruct>
{
    #region Public Properties
    public string Host
    {
    ...
    private int _reconnectDelay = 5;
    private bool _acceptData = true;
    private readonly IPackageController<TPackageStruct> _packageController;
    #endregion
    ...
    public event EventHandler<OnClientReconnectedEventArgs> OnReconnected = delegate { };
    public event EventHandler<OnClientDisconnectedEventArgs> OnDisconnected = delegate { };
    public event EventHandler<OnClientDataReceivedEventArgs<TPackageStruct>> OnDataReceived = delegate { };
    #endregion
    ...
    internal void InvokeOnReconnected(OnClientReconnectedEventArgs args) => this.OnReconnected?.Invoke(this, args);
    internal void InvokeOnDisconnected(OnClientDisconnectedEventArgs args) => this.OnDisconnected?.Invoke(this, args);
    internal void InvokeOnDataReceived(OnClientDataReceivedEventArgs<TPackageStruct> args) => this.OnDataReceived?.Invoke(this, args);
    ...
    #region Constructors
    public TcpSharpSocketClient(string host, int port, IPackageController<TPackageStruct> packageController)
    {
        this.Host = host;
        this.Port = port;
    ...
    {
        if (!this.Connected) return 0;
        var bytes = _packageController.Serialize(package);
        var sent = this._socket.Send(bytes);
        this.BytesSent += sent;
        return sent;
    }

    public async Task<long> SendPackageAsync(TPackageStruct package,CancellationToken token)
    {
        token.ThrowIfCancellationRequested();
        await Task.CompletedTask;
    ...
    private void ReceiverTask()
    {
        try
        {
            ...
            while (!_cancellationToken.IsCancellationRequested)
            {
                ...
                while (accuBuffer.Count >= _packageController.HeaderLength)
                {
                    var headerData = accuBuffer.Take(_packageController.HeaderLength).ToArray();
                    var bodyLength = _packageController.ReturnBodyLength(headerData);

                    if (accuBuffer.Count >= _packageController.HeaderLength + bodyLength)
                    {
                        var packageData = accuBuffer.Take(_packageController.HeaderLength + bodyLength).ToArray();
                        var package = _packageController.Deserialize(packageData);
                        InvokeOnDataReceived(new OnClientDataReceivedEventArgs<TPackageStruct> { Package = package });
                        accuBuffer.RemoveRange(0, _packageController.HeaderLength + bodyLength);
                    }
                    else
                    {
                        break;
