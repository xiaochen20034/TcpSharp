...
public class TcpSharpSocketServer<TPacketStruct>
{
    #region Public Properties
    public bool Listening
    {
    ...
    public IPacketController<TPacketStruct> PacketController
    {
        get;
    }
    ...
    public event EventHandler<OnServerConnectedEventArgs> OnConnected = delegate { };
    public event EventHandler<OnServerDisconnectedEventArgs> OnDisconnected = delegate { };
    public event EventHandler<OnServerDataReceivedEventArgs<TPacketStruct>> OnDataReceived = delegate { };
    #endregion
    ...
    internal void InvokeOnConnected(OnServerConnectedEventArgs args) => this.OnConnected?.Invoke(this, args);
    internal void InvokeOnDisconnected(OnServerDisconnectedEventArgs args) => this.OnDisconnected?.Invoke(this, args);
    internal void InvokeOnDataReceived(OnServerDataReceivedEventArgs<TPacketStruct> args) => this.OnDataReceived?.Invoke(this, args);
    #endregion

    #region Readonly Properties
    private TcpListener _listener;
    private readonly SnowflakeGenerator _idGenerator;
    private readonly ConcurrentDictionary<string, ConnectedClient<TPacketStruct>> _clients;
    #endregion
    ...
    #region Constructors
    public TcpSharpSocketServer(int port,IPacketController<TPacketStruct>  packetController)
    {
        this.Port = port;
        this.PacketController = packetController;
        this._idGenerator = new SnowflakeGenerator();
        this._clients = new ConcurrentDictionary<string, ConnectedClient<TPacketStruct>>();
    }
    #endregion
    ...
    public ConnectedClient<TPacketStruct> GetClient(string connectionId)
    {
        // Check Point
        if (!_clients.ContainsKey(connectionId)) return null;
    ...
    public long SendPacket(string connectionId,TPacketStruct packet)
    {
        var client = GetClient(connectionId);
        return client?.SendPacket(packet) ?? 0;
    }
    ...
    private void ListeningTask()
    {
        ...
        while (!this._cancellationToken.IsCancellationRequested)
        {
            ...
            tcpClient.SendBufferSize = this.SendBufferSize;
            tcpClient.SendTimeout = this.SendTimeout;
            var nanoClient = new ConnectedClient<TPacketStruct>(this, tcpClient, this._idGenerator.GenerateId().ToString());
            this._clients[nanoClient.ConnectionId] = nanoClient;
