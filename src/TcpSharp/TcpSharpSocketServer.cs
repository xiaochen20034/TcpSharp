...
public class TcpSharpSocketServer<TPackageStruct>
{
    #region Public Properties
    public bool Listening
    {
    ...
    public IPackageController<TPackageStruct> PackageController
    {
        get;
    }
    ...
    public event EventHandler<OnServerConnectedEventArgs> OnConnected = delegate { };
    public event EventHandler<OnServerDisconnectedEventArgs> OnDisconnected = delegate { };
    public event EventHandler<OnServerDataReceivedEventArgs<TPackageStruct>> OnDataReceived = delegate { };
    #endregion
    ...
    internal void InvokeOnConnected(OnServerConnectedEventArgs args) => this.OnConnected?.Invoke(this, args);
    internal void InvokeOnDisconnected(OnServerDisconnectedEventArgs args) => this.OnDisconnected?.Invoke(this, args);
    internal void InvokeOnDataReceived(OnServerDataReceivedEventArgs<TPackageStruct> args) => this.OnDataReceived?.Invoke(this, args);
    #endregion

    #region Readonly Properties
    private TcpListener _listener;
    private readonly SnowflakeGenerator _idGenerator;
    private readonly ConcurrentDictionary<string, ConnectedClient<TPackageStruct>> _clients;
    #endregion
    ...
    #region Constructors
    public TcpSharpSocketServer(int port,IPackageController<TPackageStruct>  packageController)
    {
        this.Port = port;
        this.PackageController = packageController;
        this._idGenerator = new SnowflakeGenerator();
        this._clients = new ConcurrentDictionary<string, ConnectedClient<TPackageStruct>>();
    }
    #endregion
    ...
    public ConnectedClient<TPackageStruct> GetClient(string connectionId)
    {
        // Check Point
        if (!_clients.ContainsKey(connectionId)) return null;
    ...
    public long SendPackage(string connectionId,TPackageStruct package)
    {
        var client = GetClient(connectionId);
        return client?.SendPackage(package) ?? 0;
    }

    public async Task<long> SendPackageAsync(string connectionId, TPackageStruct package, CancellationToken token)
    {
        token.ThrowIfCancellationRequested();
        await Task.CompletedTask;
        return SendPackage(connectionId, package);
    }
    ...
    private void ListeningTask()
    {
        ...
        while (!this._cancellationToken.IsCancellationRequested)
        {
            ...
            tcpClient.SendBufferSize = this.SendBufferSize;
            tcpClient.SendTimeout = this.SendTimeout;
            var nanoClient = new ConnectedClient<TPackageStruct>(this, tcpClient, this._idGenerator.GenerateId().ToString());
            this._clients[nanoClient.ConnectionId] = nanoClient;
